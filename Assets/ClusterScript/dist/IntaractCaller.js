...
$.onUpdate((deltaTime) => {
    ...
    const visiblePlayers = allPlayers.filter(n => 
        !newItemUsePlayers.find(item => item.playerHandle.id === n.id)
    );
    
    $.setVisiblePlayers(visiblePlayers);
    //$.log(JSON.stringify(visiblePlayers)+","+JSON.stringify(newItemUsePlayers))
