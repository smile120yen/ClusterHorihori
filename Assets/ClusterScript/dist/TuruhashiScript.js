...
$.onReceive(
	(messageType, arg, sender) => {
		...
		if (messageType === "Attack") {
			$.state.overlapItems = [];
			$.state.ClashTime = 0.3;
			if ($.state.playerHandle != null) $.state.playerHandle.setMoveSpeedRate(0.2);
		}

		if (messageType === "EndAttack") {
			if ($.state.playerHandle != null) $.state.playerHandle.setMoveSpeedRate(1);
		}
