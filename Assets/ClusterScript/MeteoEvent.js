...
$.onUpdate((deltaTime) => {
	...
	if (lockonMarkerVisibleChecker.getEnabled() && !$.state.visibleMarker) {
		...
		!lockonMarkerVisibleChecker.getEnabled() &&
		$.state.visibleMarker
	) {
		DisableMarker();
	}
	...
	if (crashStartChecker.getEnabled() && !$.state.isStartMeteoClashEvent) {
		...
		!crashStartChecker.getEnabled() &&
		$.state.isStartMeteoClashEvent
	) {
		$.state.isStartMeteoClashEvent = false;
	}
...
const AddMarkerVisiblePlayer = () => {
	let markerVisiblePlayerList = $.state.markerVisiblePlayerList.filter(
		(player) => player.exists()
	);

	// ワールド内の全プレイヤーのうち、適用済み一覧に入っていないプレイヤーを取得
	let players = $.getPlayersNear($.getPosition(), Infinity);
	let newVisiblePlayers = players.filter(
		(player) =>
			!markerVisiblePlayerList.some(
