...
const itemUsingPlayerText = $.subNode("ItemUsingPlayerText").getUnityComponent(
	"Text"
);
	...
	(requestName, arg, sender) => {
		...
		if (requestName === "itemChecked") {
			if (arg.maxDuration != -1) {
				...
				itemUsingPlayerText.unityProp.text =
					sender.userDisplayName + "が使用中";
			} else {
				cancelSound.play();
			}
		}
...
$.onUpdate((deltaTime) => {
	...
	if ($.state.removeAllDummyItemWaitTime > 0)
...
const OnMergeComplete = () => {
	...
	if ($.state.isMergeSuccess) {
		...
		finishingProductItem.maxDuration =
			10 + usedItemList[0].maxDuration + usedItemList[1].maxDuration;
		finishingProductItem.duration = finishingProductItem.maxDuration;
		finishingProductItem.price = Math.floor(
			(usedItemList[0].price + usedItemList[1].price) * 2.5
		);
		finishingProductItem.rarity = Math.max(
			usedItemList[0].rarity,
			usedItemList[1].rarity
		);
		finishingProductItem.totalMergeCount =
			(usedItemList[0].totalMergeCount ?? 0) +
			(usedItemList[1].totalMergeCount ?? 0) +
			1;
		...
		for (specialEffectData of usedItemList[1].specialEffect) {
			const index = finishingProductItem.specialEffect.findIndex(
				(item) => item.effectName == specialEffectData.effectName
			);
			if (index != -1) {
				finishingProductItem.specialEffect[index].power +=
					specialEffectData.power;
				if (finishingProductItem.specialEffect[index].power > 5)
					finishingProductItem.specialEffect[index].power = 5;
			} else {
		...
		for (specialEffectData of finishingProductItem.specialEffect) {
			switch (specialEffectData.effectName) {
				case "ドロップ鉱石強化":
					...
					break;
				case "移動速度アップ":
					finishingProductItem.baseMovementSpeed +=
						0.4 * specialEffectData.power;
					break;
				case "ドロップ確率アップ":
				...
				case "採掘速度アップ":
					...
					break;
				case "[固有]発動機":
					finishingProductItem.multipleAttackCount +=
						1 * specialEffectData.power;
					break;
			}
		}
...
const UpdateCostText = () => {
	if ($.state.completeMerge && $.state.isMergeSuccess) {
		...
		costText.unityProp.text =
			"合成費用：" + mergeCost + " 破壊確率：" + breakChance + "%";
	} else {
		const itemList = $.state.usedItemList;
...
const UpdateTuruhashiStatusText = () => {
	...
	const turuhashiStatusText1 = $.subNode(
		"TuruhashiStatusText1"
	).getUnityComponent("Text");
	const turuhashiStatusText2 = $.subNode(
		"TuruhashiStatusText2"
	).getUnityComponent("Text");
